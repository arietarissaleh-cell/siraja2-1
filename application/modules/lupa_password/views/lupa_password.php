...
<body class="min-h-screen bg-cover bg-center bg-no-repeat flex items-center justify-center px-4 overflow-x-hidden">
    <div class="flex flex-col justify-center min-h-screen w-full">
        <main class="mx-auto backdrop-blur rounded-xl max-w-md w-full p-8 shadow-lg text-gray-800 bg-white/70">
            ...
            <form id="submitForm" class="form-horizontal">
                <label for="t_nik" class="block mb-1 text-sm font-medium">Nik</label>
                ...
                <label for="t_no_wa" class="block mb-1 text-sm font-medium">No Wa</label>
                ...
                <a href="javascript:void(0);"
                    class="w-full bg-gray-900 text-white font-semibold py-3 rounded-md hover:bg-gray-800 transition flex justify-center items-center gap-2" onclick="save();">
                    KIRIM
    ...
    <script type="text/javascript">
        function save() {
            ...
            $('#error_cont').hide();
            $.post(site_url + "lupa_password/save", $('#submitForm').serialize(), function(result) {
                if (result.error) {
