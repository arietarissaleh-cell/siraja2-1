<script type="text/javascript">
	...
	{
		showProgres();
		$.post(site_url+'proses_tl/proses_tl/page'
				,{}
				,function(result) {
...
function loadPageList(pg)
	{
		showProgres();
		$.post(site_url+'proses_tl/proses_tl/page/'+pg
				,$("#dataForm").serialize()
				,function(result) {
					//$('#main-list').show();
					$('#content-list').html(result);
	...
	function contentFilterShow()
	{
		...
		showProgres();
		$.post(site_url+'proses_tl/proses_tl/filter'
				,{}
				,function(result) {
					//contentHide();
					$('#tabFilter').show();
	...
	{
		loadInput('proses_tl/input/'+id+'/'+unit);
	
	}
	
		function detailData(unit,id)
	{
		loadInput('proses_tl/detail/'+unit+'/'+id);
	...
	function deleteConfirm(id)
	{
		bootbox.confirm('Anda yakin data dengan NIK: <br>'+$('#username_'+id).val()+' akan di Hapus?', 
			function(e){
				if (e){
					$.post(site_url+"proses_tl/proses_tl/delete/"+id
						,{}
						,function(result) {
							if (result.error)
							{
