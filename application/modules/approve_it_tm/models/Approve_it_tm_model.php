...
class Approve_it_tm_model extends Base_Model {
	...
	private function _get_datatables_query($id_unit){
		...
		$this->db->select("tbl.*, kry.nama_depan, bag.deskripsi as nama_bagian ");

		$this->db->from($this->schema.".".$this->table. " tbl");

		$this->db->join("public.mst_karyawan_sdm kry", "tbl.pembuat = kry.nik", "LEFT");
		//$this->db->join("public.mst_karyawan_sdm kry", "tbl.pembuat = kry.nik", "LEFT");
		$this->db->join("public.mst_lookup bag", "tbl.bagian = bag.kode_lookup and group_lookup = 'bagian'", "LEFT");
		...
		$this->db->where("tbl.nik_kabag_it", $nik);
	...
	public function post_wa($id_tm){
		...
		$query_data_penerima = $this->db->query("SELECT
			...
			,tbl.create_by
			,tbl.catatan_atasan
			,tbl.catatan_kabag_it
			
			,kry.telp_hp
			FROM dtl_task_manager tbl
			LEFT JOIN mst_karyawan_sdm kry
			ON tbl.pic = kry.nik and tbl.fid_unit = kry.fid_lokasi_kerja and kry.status_kry_aktif = 1
		...
		foreach($query_data_penerima->result() as $row){
			...
			$catatan_kabag_it	= $row->catatan_kabag_it;
			
			$telp_hp			= $row->telp_hp;
			
			$message_anggaan="Anda Mendapatkan Task Management :
			...
			Catatan Atasan  : *$catatan_atasan* 
			Catatan	Kabag IT: *$catatan_kabag_it* 
