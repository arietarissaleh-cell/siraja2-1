<script type="text/javascript">
	...
	{
		showProgres();
		$.post(site_url+'karyawan/karyawan/page'
				,{}
				,function(result) {
...
function loadPageList(pg)
	{
		showProgres();
		$.post(site_url+'karyawan/karyawan/page/'+pg
				,$("#dataForm").serialize()
				,function(result) {
					//$('#main-list').show();
					$('#content-list').html(result);
	...
	function contentFilterShow()
	{
		...
		showProgres();
		$.post(site_url+'karyawan/karyawan/filter'
				,{}
				,function(result) {
					//contentHide();
					$('#tabFilter').show();
	...
	{
		loadInput('karyawan/input/'+unit+'/'+id);
	
	}
	
		function detailData(unit,id)
	{
		loadInput('karyawan/detail/'+unit+'/'+id);
	...
	function deleteConfirm(id)
	{
		bootbox.confirm('Anda yakin data dengan NIK: <br>'+$('#username_'+id).val()+' akan di Hapus?', 
			function(e){
				if (e){
					$.post(site_url+"karyawan/karyawan/delete/"+id
						,{}
						,function(result) {
							if (result.error)
							{
	...
	<div class="widget widget-inverse" id="main-list">
		...
		<div class="widget-head">
			<div class="btn-group pull-right">
				...
				<!-- <a class="btn btn-inverse" title="Tambah" href="javascript:void(0);" onClick="loadInput('karyawan/input')"><i class="fa fa-plus-square"></i></a> -->
