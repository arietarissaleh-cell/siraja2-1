<script type="text/javascript">
	...
	{
		showProgres();
		$.post(site_url+'approve_atasan_tm/approve_atasan_tm/page'
				,{}
				,function(result) {
...
function loadPageList(pg)
	{
		showProgres();
		$.post(site_url+'approve_atasan_tm/approve_atasan_tm/page/'+pg
				,$("#dataForm").serialize()
				,function(result) {
					//$('#main-list').show();
					$('#content-list').html(result);
	...
	function contentFilterShow()
	{
		...
		showProgres();
		$.post(site_url+'approve_atasan_tm/approve_atasan_tm/filter'
				,{}
				,function(result) {
					//contentHide();
					$('#tabFilter').show();
	...
	function inputData(id,unit)
	{
		loadInput('approve_atasan_tm/input/'+id+'/'+unit);
	
	}
	
		function detailData(id)
	{
		loadInput('approve_atasan_tm/detail/'+id);
	...
	function deleteConfirm(id)
	{
		bootbox.confirm('Anda yakin data dengan NIK: <br>'+$('#username_'+id).val()+' akan di Hapus?', 
			function(e){
				if (e){
					$.post(site_url+"approve_atasan_tm/approve_atasan_tm/delete/"+id
						,{}
						,function(result) {
							if (result.error)
							{
