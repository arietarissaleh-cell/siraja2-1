...
class Ajuan_tm_model extends Base_Model {
	...
	private function _get_datatables_query($id_unit){
		...
		$this->db->join("public.mst_karyawan_sdm kry", "kry.nik = tbl.pembuat", "LEFT");
	...
	function getDetail($where) 
	{
		...
		$this->db->select('tbl.*
						  , kry.nama_depan
						  , ats.nama_depan as nama_atasan
						  , it.nama_depan as nama_kbg_it
						  , kry_it.nama_depan as nama_pic
		...
		$this->db->join('public.mst_karyawan_sdm it','tbl.nik_kabag_it = it.nik','left');
		$this->db->join('public.mst_karyawan_sdm kry_it','tbl.pic = kry_it.nik','left');
	...
	public function post_wa_persetujuan_kabag($kode){
		...
		$query_data_penerima = $this->db->query("SELECT
			...
			ON tbl.atasan = kry.nik and tbl.fid_unit = kry.fid_lokasi_kerja and kry.status_kry_aktif = 1

			
			
			where tbl.kode_tm= '".$kode."'  
			"
		);
		...
		foreach($query_data_penerima->result() as $row){
			...
			$message_anggaan="Anda Mendapatkan Persetujuan Task Managemen :
