<script type="text/javascript">
	...
	{
		showProgres();
		$.post(site_url+'ajuan_tm/ajuan_tm/page'
				,{}
				,function(result) {
...
function loadPageList(pg)
	{
		showProgres();
		$.post(site_url+'ajuan_tm/ajuan_tm/page/'+pg
				,$("#dataForm").serialize()
				,function(result) {
					//$('#main-list').show();
					$('#content-list').html(result);
	...
	function contentFilterShow()
	{
		...
		showProgres();
		$.post(site_url+'ajuan_tm/ajuan_tm/filter'
				,{}
				,function(result) {
					//contentHide();
					$('#tabFilter').show();
	...
	{
		loadInput('ajuan_tm/input/'+unit+'/'+id);
	
	}
	
		function detailData(id,unit)
	{
		loadInput('ajuan_tm/detail/'+id+'/'+unit);
	...
	function deleteConfirm(id)
	{
		bootbox.confirm('Anda yakin data dengan NIK: <br>'+$('#username_'+id).val()+' akan di Hapus?', 
			function(e){
				if (e){
					$.post(site_url+"ajuan_tm/ajuan_tm/delete/"+id
						,{}
						,function(result) {
							if (result.error)
							{
