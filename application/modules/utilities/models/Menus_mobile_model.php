...
class Menus_mobile_model extends Base_Model {

    function __construct() {

        parent::__construct();
		$this->set_schema('hrms');
		$this->set_table('app_menu_mobile');
...
FROM (SELECT * FROM hrms.app_menu_mobile mn WHERE fid_app_menu = 0) pkey_tbl
LEFT JOIN (SELECT mn.title,mn.id_app_menu,prv.id_privilege as priv_menu
			FROM hrms.app_menu_mobile mn
			LEFT JOIN (SELECT * FROM hrms.ms_operator_privilege_mobile WHERE fid_operator = '$id_operator') prv ON mn.id_app_menu = prv.fid_app_menu
...
LEFT JOIN (SELECT mn.title,mn.fid_app_menu,prv.id_privilege_shortcut as priv_menu_shortcut,mn.order_by
			FROM hrms.app_menus_shortcut_mobile mn
			LEFT JOIN (SELECT * FROM hrms.app_menus_shortcut_priv_mobile WHERE fid_operator = '$id_operator') prv ON mn.id_app_menu_shortcut = prv.fid_app_menu_shortcut
...
EOT;
	...
	{       
		...
		$this->db->join('(SELECT * 
							FROM hrms.ms_operator_privilege_mobile
