<script type="text/javascript">
	...
	{
		showProgres();
		$.post(site_url+'utilities/operator_mobile/page'
				,{}
				,function(result) {
	...
	{
		//showProgres();
		$.post(site_url+'utilities/operator_mobile/page/'+pg
				,$("#dataForm").serialize()
				,function(result) {
					//$('#main-list').show();
					$('#content-list').html(result);
	...
	function contentFilterShow()
	{
		...
		showProgres();
		$.post(site_url+'utilities/operator_mobile/filter'
				,{}
				,function(result) {
					//contentHide();
					$('#tabFilter').show();
	...
	{
		loadInput('operator_mobile/input/'+unit+'/'+id);
	
	}
	
	function detailData(unit,id)
	{
		loadInput('operator_mobile/detail/'+unit+'/'+id);
	
	}
	
	function deleteConfirm(id)
	{
		bootbox.confirm('Anda yakin data dengan ID Operator: <br>'+$('#username_'+id).val()+' akan di Hapus?', 
			function(e){
				if (e){
					$.post(site_url+"utilities/operator_mobile/delete/"+id
						,{}
						,function(result) {
							if (result.error)
							{
	...
	{
		bootbox.confirm('Anda yakin data dengan ID Operator: <br>'+$('#username_'+id).val()+' akan dibatalkan?', 
			function(e){
				if (e){
					$.post(site_url+"utilities/operator_mobile/cancel/"+id
						,{}
						,function(result) {
							if (result.error)
							{
	...
	<div class="widget widget-inverse" id="main-list">
		...
		<div class="widget-head">
			<div class="btn-group pull-right">
				...
				<a class="btn btn-inverse" title="Tambah" href="javascript:void(0);" onClick="loadInput('utilities/operator_mobile/input')"><i class="fa fa-plus-square"></i></a>
