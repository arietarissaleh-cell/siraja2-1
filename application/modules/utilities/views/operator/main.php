<script type="text/javascript">
	...
	{
		showProgres();
		$.post(site_url+'utilities/operator/page'
				,{}
				,function(result) {
	...
	{
		//showProgres();
		$.post(site_url+'utilities/operator/page/'+pg
				,$("#dataForm").serialize()
				,function(result) {
					//$('#main-list').show();
					$('#content-list').html(result);
	...
	function contentFilterShow()
	{
		...
		showProgres();
		$.post(site_url+'utilities/operator/filter'
				,{}
				,function(result) {
					//contentHide();
					$('#tabFilter').show();
	...
	{
		loadInput('operator/input/'+unit+'/'+id);
	
	}
	
	function detailData(unit,id)
	{
		loadInput('operator/detail/'+unit+'/'+id);
	...
	function deleteConfirm(id)
	{
		bootbox.confirm('Anda yakin data dengan ID Operator: <br>'+$('#username_'+id).val()+' akan di Hapus?', 
			function(e){
				if (e){
					$.post(site_url+"utilities/operator/delete/"+id
						,{}
						,function(result) {
							if (result.error)
							{
	...
	function cancelConfirm(id)
	{
		bootbox.confirm('Anda yakin data dengan ID Operator: <br>'+$('#username_'+id).val()+' akan dibatalkan?', 
			function(e){
				if (e){
					$.post(site_url+"utilities/operator/cancel/"+id
						,{}
						,function(result) {
							if (result.error)
							{
