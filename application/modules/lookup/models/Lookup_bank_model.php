<?php
class Lookup_bank_model extends Base_Model {
	var $column_order_list_bank = array(null, 'nama_bank', null);
	var $column_search_list_bank = array('UPPER(tbl.nama_bank)');

	var $order_list_bank = array('nama_bank' => 'asc');

	private function _get_datatables_query_list_bank(){
		$this->db->select("tbl.id_bank, tbl.nama_bank");
		$this->db->from("public.mst_bank_mitra tbl");
		$this->db->order_by("tbl.nama_bank", "ASC");
